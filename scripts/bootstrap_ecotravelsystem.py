#!/usr/bin/env python3
"""Bootstrap EcoTravelSystem on zhiraf-b2b.ru: theme, pages, menus, media import."""
...
$canonical = 'https://zhiraf-b2b.ru';
update_option('home', $canonical);
update_option('siteurl', $canonical);

switch_theme('ecotravelsystem');

$pages = [
    ['Организация командировок для бизнеса в Уфе и РФ', 'glavnaya', 'Профессиональный тревел-менеджмент для компаний: авиа, ж/д, отели, визы и MICE.', ''],
    ['Услуги корпоративного тревел-менеджмента', 'uslugi', 'Услуги TMC: тревел-менеджмент визы MICE | Жираф B2B', 'page-uslugi.php'],
    ['Корпоративным клиентам: как мы работаем', 'klientam', 'Подключение TMC: договор заявки отчётность закрывающие документы поддержка 24/7.', 'page-klientam.php'],
    ['Частые вопросы о корпоративных командировках и TMC', 'faq', 'Ответы ЛПР: чем TMC лучше самостоятельного бронирования как формируется стоимость какие документы поддержка 24/7.', 'page-faq.php'],
    ['Контакты TMC «Жираф» в Уфе', 'kontakty', 'Офис в Уфе: ул. Гагарина 74/1. Телефон email Telegram WhatsApp.', 'page-kontakty.php'],
    ['О компании «Жираф» — TMC из Уфы с охватом по России', 'o-nas', 'Экспертное TMC из Уфы: персональный менеджер сложные маршруты прозрачная отчётность.', 'page-o-nas.php'],
    ['Блог о корпоративных командировках и TMC', 'blog', 'Экспертные материалы для HR CFO и администраторов: бюджет документы тренды TMC.', ''],
...
update_option('blogname', 'EcoTravelSystem');
update_option('blogdescription', 'Корпоративный тревел-менеджмент под ключ');
...
def build_manifest_from_media_map() -> dict:
    ...
    for item in media_map.get("assets", []):
        ...
        if not file_name or Path(file_name).is_absolute() or ".." in Path(file_name).parts or not alt:
            # Skip if no alt, but ecotravelsystem has alts in media-map.json
            continue
        assets.append(
            {
                "id": registry_id,
...
def upload_and_run_php(env: dict, php: str, remote_name: str) -> str:
    ...
    ftp.set_pasv(True)
    ftp.cwd("/")
    # Try to find public_html or similar if not in root
...
def main() -> int:
    ...
    out = upload_and_run_php(env, php, "ets-aurora-bootstrap-once.php")
    print(out)
    ...
    css_url = f"{PUBLIC_URL}wp-content/themes/{THEME_SLUG}/style.css"
    ...
    deploy_log = f"""# Deploy Log — EcoTravelSystem
...
## Asset transport
- Preflight: `asset_transport.py` — PASS
- Report: `teya-memory/wp/asset-transport-report.md`

## FTP deploy
- Uploaded theme files to `/wp-content/themes/{THEME_SLUG}`
...
| Check | Result |
| --- | --- |
| HTTPS homepage | status={https_status}, final={https_final}, body_len={https_len} |
...
| WP media imported | {asset_count} assets |
...
**Project:** EcoTravelSystem
**Theme:** {THEME_SLUG}
**Public URL:** {PUBLIC_URL}
**Status:** {"PASS" if deploy_ok else "BLOCKER"}
**Generated:** {now}

- Asset transport: PASS
- FTP deploy: /wp-content/themes/{THEME_SLUG}
- Bootstrap: theme activated, pages created, menu set
- WP Media: {asset_count} attachments
